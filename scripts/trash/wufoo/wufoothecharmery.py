import re
from os.path import basename

import requests

from module import Spam


def get(session: requests.Session):
    headers = {
        'authority': 'wonderpaws.wufoo.com',
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7',
        'cache-control': 'max-age=0',
        'sec-ch-ua': '"Chromium";v="110", "Not A(Brand";v="24", "Google Chrome";v="110"',
        'sec-ch-ua-mobile': '?0',
        'sec-ch-ua-platform': '"Windows"',
        'sec-fetch-dest': 'document',
        'sec-fetch-mode': 'navigate',
        'sec-fetch-site': 'none',
        'sec-fetch-user': '?1',
        'upgrade-insecure-requests': '1',
        'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Safari/537.36',
    }

    response = session.get(
        'https://thecharmery.wufoo.com/forms/join-team-charmery-scooper-w1xvqt2e1v9lz82/',
        headers=headers
    )
    return response


def post(session: requests.Session, id_stamp, text, target):
    headers = {
        'authority': 'thecharmery.wufoo.com',
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'accept-language': 'ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7',
        'cache-control': 'max-age=0',
        'content-type': 'multipart/form-data; boundary=----WebKitFormBoundaryIg4WodEBKDJdKoyq',
        'origin': 'https://thecharmery.wufoo.com',
        'referer': 'https://thecharmery.wufoo.com/forms/join-team-charmery-scooper-w1xvqt2e1v9lz82/',
        'sec-ch-ua': '"Chromium";v="110", "Not A(Brand";v="24", "Google Chrome";v="110"',
        'sec-ch-ua-mobile': '?0',
        'sec-ch-ua-platform': '"Windows"',
        'sec-fetch-dest': 'document',
        'sec-fetch-mode': 'navigate',
        'sec-fetch-site': 'same-origin',
        'sec-fetch-user': '?1',
        'upgrade-insecure-requests': '1',
        'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Safari/537.36',
    }

    data = '------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="Field1"\r\n\r\ntest\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="Field2"\r\n\r\ntest\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="Field4"\r\n\r\nwezxasqw@gmail.com\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="Field5"\r\n\r\n122\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="Field5-1"\r\n\r\n212\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="Field5-2"\r\n\r\n2121\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="Field7"\r\n\r\n\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="Field7"\r\n\r\nYes\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="Field222"\r\n\r\nIce Cream Factory (1700 W 41st St) \r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="Field19"\r\n\r\nPT - 15 to 25 Hours / Week (3 to 4 shifts per week) (Weekend & Nights)\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="Field426"\r\n\r\nDaytime (11am to 5pm)\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="Field16"\r\n\r\ntest\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="Field12"\r\n\r\ntest\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="Field323"\r\n\r\ntest\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="Field322"\r\n\r\ntest\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="Field321"\r\n\r\ntest\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="currentPage"\r\n\r\nkawuBeJoxDaeNrCerZgmiMLdJaieEM9pfwuBe2uuSwuBeB1ArskU=\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="saveForm"\r\n\r\nSubmit\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="comment"\r\n\r\n\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="idstamp"\r\n\r\nidstamp_field\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="encryptedPassword"\r\n\r\n\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="stats"\r\n\r\n{"errors":0,"startTime":2656,"endTime":16085,"referer":null}\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq\r\nContent-Disposition: form-data; name="clickOrEnter"\r\n\r\nclick\r\n------WebKitFormBoundaryIg4WodEBKDJdKoyq--\r\n'

    data = data.replace('idstamp_field', id_stamp).replace('test', text).replace('wezxasqw@gmail.com', target)

    response = session.post(
        headers['referer'],
        headers=headers,
        data=data.encode(),
    )
    return response


class ConcreteSpam(Spam):

    def post(self, target) -> requests.Response | None:
        s = requests.Session()
        s.proxies = self.get_proxies()
        get_resp = get(s)
        pattern = re.compile('(?<=name="idstamp" value=").*(?=" >)')
        idstamp = pattern.findall(get_resp.text)
        if not idstamp:
            return
        text = self.get_text()
        post_resp = post(s, idstamp[0], text, target)
        return post_resp


def main():
    spam = ConcreteSpam(basename(__file__).removesuffix('.py'), 'Thank You')
    res = spam.send_post()
    if res:
        spam.run_concurrently(30)


if __name__ == '__main__':
    main()
